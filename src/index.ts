export function convertNumberToEnglishText(n: number): string {
    ...
    let nAbs : number = Math.abs(n);
    let nTxt : string = nAbs.toString();
    let numAsTxt : string = '';

    // Helper functions

    function digit(idx: number) : number { // Workaround for Typescript throwing error when using string as array index value
        return parseInt(nTxt[idx]);
    }

    function checkToContinue(arg: number) { // Function that checks if numbers > 99 have anything bellow thousands/hundreds
        let remain : number = parseInt(nTxt.slice(arg));
        if (remain != 0) {
    ...
    if (!Number.isInteger(n)) return 'Number is not an integer!';

    // Checking if number does not exceed maximum digit count (5 digits or less)
    if (nTxt.length > 5) return 'The number exceeds maximum digit count.';
    ...
    if (n === 0) return 'zero'; // Case of n value = 0

    if (n < 0) numAsTxt+= 'negative '; // Check if number is negative/positive
    ...
    switch (nTxt.length) {
        ...
        case 2: // Case for two-digit numbers
            if (nAbs < 20) { // Case for n values [-19,-1; 1,19]
                ...
                if (digit(1) != 0) numAsTxt+= ' ' + units[digit(1)];
            }
            break;
    ...
    if (numAsTxt.endsWith(' ')) numAsTxt = numAsTxt.slice(0, -1);
    return numAsTxt;
}